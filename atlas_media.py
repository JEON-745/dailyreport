"""아틀라스 최종 리포트 자동 입력 엔진 (NSR형: 단일 월시트 + 가로블록 구조).

NSR 엔진(nsr_media.py) 클론. 아틀라스 차이점만 반영:
- 매체 블록: 파워링크 PC / 파워링크 MO / GFA 웹사이트 전환 광고 / SNS 광고(META).
  (브검·쇼핑검색·크리테오·모비온 없음)
- 소스 시트명: '파워링크'(블록 파워링크_PC/파워링크_MO), 'GFA'(단일 블록).
- 소재별 효율 시트명: '메타 소재별 효율_6월'처럼 월 접미사가 붙음 -> 자동 감지.
- 소재 매칭 키: D열(기획전_명) + E열(번호) 조합. raw 광고이름 split('_')[2:]와 일치
  (예: '0611_트래픽_나이트런_003' -> '나이트런_003' = D'나이트런'+E'003').

대상(타겟): 아틀라스 통합보고서의 월시트(예: '2026_6월')와 '메타 소재별 효율_*' 시트.
소스: 엠케이로드 통합보고서(파워링크·GFA 시트) + META 소재별 raw('Raw Data Report', '일'열 포함).
...
MONTH_BLOCKS = [
    ("파워링크 PC",          "파워링크", "파워링크_PC", "광고비"),
    ("파워링크 MO",          "파워링크", "파워링크_MO", "광고비"),
    ("GFA 웹사이트 전환 광고", "GFA",      None,          "광고비"),
    # 'SNS 광고'(META 일별)는 run()에서 META raw로 별도 처리.
]
...
def soso_rowmap(ws, r0, r1):
    """블록 내 소재 키(D 기획전_명 + E 번호) -> 행.
    D열은 그룹 첫 행에만 있으므로 직전 D를 이어받음(carry-forward).
    원본 D·E가 모두 빈 행(TOTAL/빈행)은 제외. 키는 raw의 '기획전명_번호'와 일치.
    """
    rows = {}
    cur_d = None
    for r in range(r0, r1 + 1):
        d = ws.cell(r, 4).value  # D 기획전_명
        e = ws.cell(r, 5).value  # E 기획전 명(번호)
        d = str(d).strip() if d is not None and str(d).strip() else None
        e = str(e).strip() if e is not None and str(e).strip() else None
        if d:
            cur_d = d
        if not d and not e:
            continue  # TOTAL/빈행
        if not cur_d:
            continue
        key = f"{cur_d}_{e}" if e else cur_d
        if key not in rows:
            rows[key] = r
    return rows
...
def run(tonghap_path, meta_path, target_path, out_path, only_dates=None, month_sheet=None):
    ...
    # 소재별 효율 시트 자동 감지('메타 소재별 효율' 또는 '메타 소재별 효율_6월' 등)
    soso_sheet = next((s for s in twb.sheetnames
                       if s.replace(" ", "").startswith("메타소재별효율")), None)
    ...
    if soso_sheet:
        ...
        if blk:
            hr, sd, ed, r0, r1 = blk
            label_ok = sd <= fill_date <= ed
            # 라벨이 채울날짜를 포함하면 블록 종료일까지, 아니면(중복/오라벨 폴백)
            # 채울날짜까지 누적. raw는 해당 주차 export라 어느 쪽이든 주차누적과 같음.
            upto = ed if label_ok else fill_date
            rowmap = soso_rowmap(swb_data, r0, r1)
            groups = meta_creative_groups(meta_ws, cm, upto_date=upto)
            ...
            for key, m in groups.items():
                ...
                n_soso += 1
            warn = "" if label_ok else f"  ⚠ 블록 라벨({sd}~{ed})이 채울날짜({fill_date})를 포함하지 않음(폴백 선택)"
            log.append(f"[소재별] 주차 {sd}~{ed} (행{r0}~{r1}): {n_soso}소재 입력, 신규 {len(new_creatives)}건{warn}")
